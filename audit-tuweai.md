...
- ✅ corregido: la política comercial de moneda ya quedó explicitada sin inventar un checkout global; `client/src/features/payments/components/pricing-section.tsx` mantiene USD como referencia de alcance y `client/src/features/proposals/components/proposal-request-page.tsx` aclara que, para Argentina, la propuesta final se cotiza y se cobra en pesos argentinos.
- ✅ corregido: las tres cards de `client/src/features/payments/components/pricing-section.tsx` ya no se presentan como bloques altos y desparejos; ahora comparten la misma estructura visual, limitan el contenido a un set mas compacto de beneficios, destacan la card central como recomendada y explicitan un beneficio concreto (`hosting + dominio profesional por 1 año`) sin romper la paleta ni la lectura responsive del landing.
- ✅ corregido: `client/src/features/payments/components/pricing-section.tsx` ya no obliga a pasar por `/consulta` para alcances base; el home vuelve a resolver checkout directo con Mercado Pago para `Base web` y `Proyecto comercial`, alinea la moneda visible con el cobro real en ARS y deja `Solución a medida` como único caso de propuesta personalizada.
- ✅ corregido: el autoscroll pegajoso ya no afecta a páginas fuera del landing; `client/src/index.css` dejó de globalizar `scroll-snap-type` y `scroll-snap-align`, y `client/src/features/marketing-home/components/marketing-home-page.tsx` mantiene el snapping solo dentro del home mediante `landing-scroll-shell`.

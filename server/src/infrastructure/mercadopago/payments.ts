...
import { mercadopagoPayment, mercadopagoPreference } from './client';
...
export const createPaymentPreference = async (plan: PaymentPlan) => {
  ...
  const mpRes = await withRetry('create_payment_preference', () =>
    withTimeout(
      mercadopagoPreference.create({ body: preferenceData as never }),
      MP_TIMEOUT_MS,
...
export const getPaymentDetails = async (paymentId: string | number) => {
  return await withRetry('get_payment_details', () =>
    withTimeout(mercadopagoPayment.get({ id: paymentId }), MP_TIMEOUT_MS, 'mercadopago_payment_get')
  );
};

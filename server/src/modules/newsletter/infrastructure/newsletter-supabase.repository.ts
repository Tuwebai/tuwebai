...
const findNewsletterSubscriberById = async (
  ...
  const rows = await supabaseAdminRestRequest<NewsletterSubscriberRow[]>(
    `/newsletter_subscribers?select=${NEWSLETTER_SELECT}&external_subscriber_id=eq.${encodeURIComponent(subscriberId)}&limit=1`,
  );
...
const updateNewsletterBrevoSyncById = async (
  ...
  await supabaseAdminRestRequest<void>(
    `/newsletter_subscribers?external_subscriber_id=eq.${encodeURIComponent(subscriberId)}`,
    {
      method: 'PATCH',
      body: JSON.stringify({
